...
# restore your libraryn (keep this at the start of every script)
renv::restore()
...
ggplot2::ggplot(data=FactCockleSize, mapping=aes(x=length_mm,y=thickness_mm)) + geom_point()

# there is an outlier, likely an data entry mistake > remove it.
FactCockleSize |> dplyr::filter(thickness_mm<500)

FactCockleSize <- readr::read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vSpormjGiM4uWMTIZYm9Upt5j1Ige_Wu9CrhGGdvjXwxmHP2R6S7JDyEmiGTn3fXihl5s5yBn_sdo8h/pub?gid=1538766002&single=true&output=csv") |> dplyr::filter(CockleObs_ID!=1531,CockleObs_ID!=469) |> dplyr::mutate(year=factor(year)) # make year a factor

FactCockleSize |> dplyr::filter(thickness_mm>10 & length_mm<5)

ggplot2::ggplot(data=FactCockleSize, mapping=aes(x=length_mm,y=thickness_mm,)) + geom_point(mapping=aes(colour=year)) + geom_smooth(method = "lm",colour='black')
...
ggplot2::ggplot(mapping=aes(x=length_mm,y=thickness_mm,)) + geom_point(mapping=aes(colour=year)) + geom_smooth(method = "lm",colour='black') + facet_wrap(~year) #make several lose year plots
