...
names(elevdat)
elevdat |> 
  ggplot(aes(x=TransectPoint_ID, y=elevation_m, colour = year,))+ geom_line(size=0.8)

# plot the change in transect  elevation along the transect, using a separate line color for each year
names(elevdat)
elevdat |> 
  ggplot(aes(x=TransectPoint_ID, y=elevation_m, colour = year,))+ geom_line(size=0.8)+ facet_wrap(~year)
...
cdat2017 <- read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vSpormjGiM4uWMTIZYm9Upt5j1Ige_Wu9CrhGGdvjXwxmHP2R6S7JDyEmiGTn3fXihl5s5yBn_sdo8h/pub?gid=1538766002&single=true&output=csv") |>
  dplyr::filter(year==2017, CockleObs_ID!= 465, CockleObs_ID!=1531)|>
  dplyr::group_by(TransectPoint_ID)|>
  dplyr::summarize(n_obs=n(), avg_l = mean(length_mm, na.rm=T), sd_l=sd(length_mm, na.rm=T), se_l=sd_l/sqrt(n_obs))
...
cdat2017 |> ggplot2::ggplot(aes(x=TransectPoint_ID,y=n_obs))+ geom_point()+ geom_line()
...
combidat |> ggplot2::ggplot(aes(x=elevation_m,y=n_obs))+ geom_point()+ geom_smooth(method = "loess")
